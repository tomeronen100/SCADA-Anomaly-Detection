...
class EFDDiscretizer:
    """Equal Frequency Discretization - divides data into bins with equal number of samples"""
    ...
    def fit(self, data):
        ...
        # Use quantiles to create equal frequency bins
        quantiles = np.linspace(0, 100, self.n_bins + 1)
        self.bin_edges = np.percentile(data, quantiles)
...
def process_data(train_csv_path: str, test_csv_path: str, n_bins: int = 10) -> Tuple[pd.DataFrame, pd.DataFrame, Dict, Dict]:
    ...
    Args:
        ...
        n_bins: Number of bins for EFD discretization (default: 10)
    
    Returns:
        train_processed: Processed training data
        test_processed: Processed test data
        discretizers: Dictionary of fitted EFD discretizers for 'Register_' columns
    ...
    # 1a. Discretize 'Register_' columns in train
    print(f"\n  1a. Discretizing 'Register_' columns in TRAIN...")
    for col in reg_columns:
        print(f"\n    Processing: {col}")
        print(f"      - Applying EFD discretization (n_bins={n_bins})")
        discretizer = EFDDiscretizer(n_bins=n_bins)
...
def save_results(train_df: pd.DataFrame, test_df: pd.DataFrame, 
    ...
    # Method name
    method_name = "EFD"  # Equal Frequency Discretization
    ...
    with open(stats_path, 'w') as f:
        ...
        for col in sorted(discretizers.keys()):
            f.write(f"\n{col}:\n")
            f.write(f"  Number of bins: {discretizers[col].n_bins}\n")
...
def main():
    ...
    for n_bins in bins_list:
        print("\n" + "="*80)
        print(f"PROCESSING WITH {n_bins} BINS")
        print("="*80 + "\n")
