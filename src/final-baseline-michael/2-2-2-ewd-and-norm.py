...
class EWDDiscretizer:
    """Equal Width Discretization - divides data into bins with equal width"""
    ...
    def fit(self, data):
        ...
        # Create n_bins + 1 edges (e.g., 4 bins needs 5 edges)
...
def process_data(train_csv_path: str, test_csv_path: str, n_bins: int = 10) -> Tuple[pd.DataFrame, pd.DataFrame, Dict, Dict]:
    ...
    Args:
        ...
        n_bins: Number of bins for EWD discretization (default: 10)
    
    Returns:
        train_processed: Processed training data
        test_processed: Processed test data
        discretizers: Dictionary of fitted EWD discretizers for 'Register_' columns (excluding '_time')
    ...
    for col in reg_columns:
        print(f"\n    Processing: {col}")
        print(f"      - Applying EWD discretization (n_bins={n_bins})")
        discretizer = EWDDiscretizer(n_bins=n_bins)
...
def save_results(train_df: pd.DataFrame, test_df: pd.DataFrame, 
    ...
    # Method name
    method_name = "EWD"  # Equal Width Discretization
...
def main():
    ...
    for n_bins in bins_list:
        print("\n" + "="*80)
        print(f"PROCESSING WITH {n_bins} BINS (EWD - Equal Width Discretization)")
        print("="*80 + "\n")
